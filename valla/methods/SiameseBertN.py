...
def main():
    ...
    parser.add_argument('--train_path', type=str,default= '../datasets/Guardian_original/processed/guardian_train.csv')
    # parser.add_argument('--test_path', type=str, default='../datasets/Guardian_original/processed/guardian_AV_test.csv')
    # parser.add_argument('--train_path', type=str,default= '../datasets/Guardian_original/processed/guardian_train.csv')
    # parser.add_argument('--test_path', type=str, default='../datasets/Guardian_original/processed/guardian_AA_cross_genre_val.csv')
    # parser.add_argument('--train_path', type=str,default= '../datasets/reuter/processed/CCAT50_train.csv')
    parser.add_argument('--test_path', type=str, default='../datasets/reuter/processed/CCAT50_AV_test.csv')
    # parser.add_argument('--train_path', type=str, default='../datasets/imdb/processed/imdb62_train.csv')
    # parser.add_argument('--actualtest_path', type=str, default='../datasets/imdb/processed/imdb62_AV_test.csv')
    # parser.add_argument('--actualtest_path', type=str, default='../datasets/Guardian_original/processed/guardian_AV_cross_genre_test.csv')
    parser.add_argument('--actualtest_path', type=str, default='../datasets/')
    ...
    with wandb.init(project=args.wandb_project, config=vars(args),name=f"Test_Experiment_AV_{args.exp_number}") as run:
        ...
        wandb.config.update({"slurm_job_id": slurm_job_id, "experiment_number": args.exp_number})
        ...
        if args.AA:
            ...
            actual_test_dataset = get_aa_dataset(args.actualtest_path)
        else:
            test_dataset = AVDataset(args.test_path)
            actual_test_dataset = AVDataset(args.actualtest_path)
        ...
        if args.AA:
            # Create a new evaluator instance for the test dataset
            test_evaluator = AaEvaluator(train_dataset=train_dset_for_authembs,
                                 test_dataset=actual_test_dataset,  # Load your actual test dataset here
                                 distance_metric=contrastive_dist_metric,
                                 triplet_dist_metric=triplet_dist_metric,
                                 num_docs_per_auth_embedding=args.num_docs_per_auth_embedding,
                                 num_chunks_per_doc_per_auth_embedding=args.num_chunks_per_doc_per_auth_embedding,
            ...
            test_evaluator = ContrastiveEvaluator(actual_test_dataset,
                                             batch_size=args.eval_batch_size,
                                             distance_metric=contrastive_dist_metric,
                                             triplet_dist_metric=triplet_dist_metric,
                                             kernel_fn=kernel_fn,
                                             post_eval_callable=post_eval_callable)
        # Call the evaluator on the test set
